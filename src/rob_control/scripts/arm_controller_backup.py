...
SER_ARM = modbus.Instrument(port='/dev/ttyS4', slaveaddress=2)
...
def serial_loop():
    ## 串口命令处理进程
    ser_4G = SerialCmd.SerialCmd(port = '/dev/ttyS5')
...
def get_goal_traj(arm, goal, axis = 'y', direction = -1):
    ...
    try:
        ...
        # 设置机械臂终端运动目标位姿的安全位姿
        if axis == 'x':
            goal_temp.position.x += (0.3*direction)
        elif axis == 'y':
            goal_temp.position.y += (0.3*direction)
        elif axis == 'z':
            goal_temp.position.z += (0.3*direction)
